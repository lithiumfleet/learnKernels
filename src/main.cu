...
__global__ void mmaAtom_m16n8k8_f32f16f16f32_TN(const half *a, const half *b,
  ...
  auto tb = make_tensor(b, make_layout(make_shape(8, 8), LayoutRight{}));
...
void gemm_ijk_nt(const half *A, const half *B, float *C, int M, int N, int K) {
  for (int i = 0; i < M; ++i) {
    for (int j = 0; j < N; ++j) {
      float acc = C[i * N + j];
      ...
      for (int k = 0; k < K; ++k) {
        ...
        float b = __half2float(B[j * K + k]); // B is N x K
        acc += a * b;
      }
...
int main() {
  ...
  gemm_ijk_nt(A.data(), B.data(), C.data(), M, N, K);
